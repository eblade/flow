...
class MetadataMapper(Flow, NeedsClient, NeedsStore, NeedsConfig):
    ...
    def start(self, asset, info=None, log_id=-1):
        for retry in range(max_retries):
            try:
                logging.info(u'(%s) Updating metadata (attempt %i of %i)...',
                             log_id, retry + 1, self.max_retries)

                # Fetch metadata with a fresh etag
                try:
                    old_metadata = Payload(client.GET(asset.describedby_link))
            ...
            # If we have a conflict, retry the entire operation
                ...
                logging.warn(u'(%s) Asset Metadata update failed %s', log_id, str(e))
                if e.response.status_code == 409: # etag violation
                    logging.warn(u'(%s) Retrying...', log_id)
                else:
                    break
